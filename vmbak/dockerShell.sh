...
for imageName in ${images_QUAY[@]} ; do
  ...
  docker tag $QUAY_URL/$imageName $LOCAL_URL/quay_io/$imageName
  docker push $LOCAL_URL/quay_io/$imageName
done

for imageName in ${images_GCR[@]} ; do
  #echo ${GCR_URL}$imageName
  docker pull ${GCR_URL}/$imageName
  docker tag $GCR_URL/$imageName $LOCAL_URL/gcr_io/$imageName
  docker push $LOCAL_URL/gcr_io/$imageName
done

for imageName in ${images_K8S[@]} ; do
  #echo ${K8S_URL}$imageName
  docker pull ${K8S_URL}/$imageName
  docker tag $K8S_URL/$imageName $LOCAL_URL/k8s_gcr_io/$imageName
  docker push $LOCAL_URL/k8s_gcr_io/$imageName
done

for imageName in ${imgaes_other[@]} ; do
  #echo $imageName
  docker pull $imageName
  docker tag $imageName $LOCAL_URL/k8s_io/$imageName
  docker push $LOCAL_URL/k8s_io/$imageName
done

docker pull $GCRX_URL/$images_GCRX
docker tag $GCRX_URL/$imageName $LOCAL_URL/gcrx_io/$imageName
docker push $LOCAL_URL/gcrx_io/$imageName